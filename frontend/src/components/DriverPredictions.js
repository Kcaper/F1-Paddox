...
function DriverPredictions() {
  ...
  const getDrivers = async () => {
    await fetch(baseUrl + '/api/predictions/season/driver-standings/' + params['id'] + '/')
    .then(response => response.json())
    .then(apiDrivers => {
      var object = JSON.parse(apiDrivers)
      updatedrivers(object.drivers)
      setUser(object.user)
      setPredictionType(object.isSeasonPrediction)
  ...
  const getDeadline = async () => {
    await fetch(baseUrl + '/api/predictions/deadlines/season/driver-standings/' + params['id'] + '/')
    .then(response => response.json())
    .then(deadline => {
      setValidator(1)
  ...
  function submitPrediction() {
    ...
    prediction_dict["isSeasonPrediction"] = 1
    ...
    if (submissionVaidator == 1){
      fetch(baseUrl + '/api/predictions/pre-season/driver-standings/submit/', {
        method: 'POST',
        headers: {
    ...
    function handleMidSeasonClick(){
      window.location.href='/mid-season-driver-predictions'
    }
    ...
    function handlePaddockChange(paddockName){
      ...
      for(let i=0; i<userPaddocks.length; i++){
        if (userPaddocks[i]['paddockName'] == paddockName){
          ...
          window.location.href = baseUrl + '/driver-predictions/' + paddockId
        }
      }
    }
            ...
            <Droppable droppableId="drivers">
              {(provided) => (
                <ul className="drivers" {...provided.droppableProps} ref={provided.innerRef} style={{
                  ...
                  {drivers.map(({id, name, icon, flag, thumb, constructor_logo, constructorName, constructorIconColor}, index) => {
                    return (
                          <li ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                            <div style={{
                              ...
                              paddingLeft: "10px",
                              paddingRight:"15px",
                              gridTemplateColumns: 'auto 45px 70px 120px 70px',
                              borderRadius: "10px",
                              alignItems: 'center',
                              flexDirection: "row",
