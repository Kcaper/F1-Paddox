...
def driverPopSerializer(uid, prediction_type, paddockId):
    ...
    if prediction_type == "pre season":
        try:
            ...
            print(potential_predictions_list)
...
def getPaddockManualDrivers(uid, paddockId):
    ...
    print(data["fastLapFound"])
    print(data["poleLapFound"])
...
def getLeaderboardByPaddock(pid):
    ...
    if file_found == False:
        ...
        print(paddock_user_dict['lastRound'])
                ...
                for r in range(race_start_round, next_race_round, 1):
                    ...
                    for driver in range(result_finishing_qset.count()):
                        ...
                        try:
                            ...
                            if user_finishing_qset[driver].pointsForPrediction == 2:
                                prediction_points = 2
                            elif singlePointPredictionHit == 0:
                                prediction_points = 0
                            elif singlePointPredictionHit == 1:
                                prediction_points = 1
                            else:
                                prediction_points = 0
...
#def getUserDriverStandingPointsByPaddock(pid):
    ...
    cwd = createJsonFolderStructure(pid, last_completed_race)
    ...
    if file_found == False:
        ...
        for user in range(0, user_qset.count(), 1):
            data['paddockUsers'].append({
                "username" : user_qset[user].user.username,
                "userId" : user_qset[user].user_id,
            })
            data["userPoints"][user_qset[user].user.username] = {}
            for r in range(1, next_race_round, 1):
                user_points_qset = predictionPoints.objects.filter(seasonCalendar_id=seasonCalendar.objects.filter(year=now.year, raceRound=r)[0].id, isFeatureRaceMidfieldPrediction=1, user_id=user_qset[user].user_id).order_by('predictedPosition')
                data["userPoints"][user_qset[user].user.username][seasonCalendar.objects.filter(year=now.year, raceRound=r)[0].circuit.circuitRef] = []
                for record in range(0, num_drivers_on_leaderboard,1):

                    #
                    if record+1 == num_drivers_on_leaderboard:
                        if user_points_qset[record].pointsForPrediction == 1:
                            if user_points_qset[record-1].driver_id == user_points_qset[record].driver_id:
                                prediction_points = 1
                            else:
                                prediction_points = 0
                        else:
                            prediction_points = user_points_qset[record].pointsForPrediction
                    else:
                        prediction_points = user_points_qset[record].pointsForPrediction

                    data["userPoints"][user_qset[user].user.username][seasonCalendar.objects.filter(year=now.year, raceRound=r)[0].circuit.circuitRef].append({
                        'predictedDriverId' : user_points_qset[record].driver_id,
                        'predictedDriverCode' : user_points_qset[record].driver.code,
                        'predictedPosition' : record+1,
                        'predictionPoints' : prediction_points,
                        'resultDriverId' : user_points_qset.filter(finishingPosition=record+1)[0].driver_id,
                        'resultDriverCode' : user_points_qset.filter(finishingPosition=record+1)[0].driver.code,
                    })
...
def getDriverStandingDataByPaddock(pid):
    ...
    if file_found == False:
        ...
        num_drivers_on_leaderbaords = drivers.objects.filter(isIncludedInPredictions=1).count()
        ...
        prediction_points_qset = predictionPoints.objects.filter(
            seasonCalendar_id=seasonCalendarId,
            isDriverStandingPrediction=1).order_by('finishingPosition')
        ...
        for l in range(0, leaderboard_qset.count(), 1):
            ...
            driver_points_qset = driverStandings.objects.filter(
                ...
                seasonCalendar_id=seasonCalendar.objects.filter(
                    year=now.year,
                    raceRound=current_round).latest('id').id
                ).order_by('position')
            ...
            for d in range(0, driver_points_qset.count(), 1):
                ...
                if d>0:
                    lastDriverId = driver_points_qset[d-1].driver_id
                ...
                if drivers.objects.get(id=driverId).isOnGrid != 1:
                    ...
                    try:
                        driver_previous_position = driverStandings.objects.filter(driver_id=driverId, seasonCalendar_id=seasonCalendar.objects.filter(year=now.year,raceRound=previous_round)[0].id)[0].position
                    except:
                ...
                try:
                    ...
                    data['predictions'][leaderboard_qset[l].user.username].append({
                        'id':driverPos,
                        'driverId':driverId,
                        'driverCode':drivers.objects.get(id=driverId).code,
                        'driverConstructorName':drivers.objects.get(id=driverId).currentTeam.name,
                        'currentPosition':driver_position,
                        'driverPoints':int(driver_points_qset[d].points),
                        'positionPredictionDriverCode':drivers.objects.get(id=user_prediction_qset.filter(predictedPosition=driver_position).latest('id').driver_id).code,
                        'predictionPoints':prediction_points,
                        'driverDeltaText':driver_delta_text,
                        'driverDelta':driver_delta,
                        'singlePointFinishingHit': predictionPoints.objects.filter(user_id=leaderboard_qset[l].user_id, driver_id=driverId).latest('id').isFinishingSinglePoint,
                        'singlePointPredictionHit': predictionPoints.objects.filter(user_id=leaderboard_qset[l].user_id, driver_id=driverId).latest('id').isPredictedSinglePoint,
                    })
                except:
                    pass
...
def getConstructorStandingDataByPaddock(pid):
    ...
    if file_found == False:
        ...
        print("THE NEXT ROUND IS " + str(next_race_round))
            ...
            for d in range(0, constructor_points_qset.count(), 1):
                ...
                try:
                    ...
                    data['predictions'][leaderboard_qset[l].user.username].append({
                        ...
                        'singlePointFinishingHit': predictionPoints.objects.filter(seasonCalendar_id = seasonCalendarId, user_id=leaderboard_qset[l].user_id, constructor_id=constructorId)[0].isFinishingSinglePoint,
                        'singlePointPredictionHit': predictionPoints.objects.filter(seasonCalendar_id = seasonCalendarId, user_id=leaderboard_qset[l].user_id, constructor_id=constructorId)[0].isPredictedSinglePoint,
                    })
                except Exception as e:
                    print(e)
