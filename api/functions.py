...
def getNextRaceRound():
    ...
    try:
        if seasonCalendar.objects.get(year=now.year, raceRound = next_race_round - 1).isComplete == 0:
            next_race_round = next_race_round - 1
    except:
        next_race_round = 1

    print("THER NEXT RACE ROUDN IS " + str(next_race_round))
...
def getDriverStandings(race_round):
    ...
    def updatePointsTable(race_round):
        ...
        for p in range(0,len(sorted_constructors_by_points_list),1):
            ...
            try:
                if drivers.objects.get(id=driverId).isIncludedInPredictions == 0:
                    continue
                else:
                    position = position + 1
            except:
            ...
            db_entry = driverStandings(id=id_type,
                points=instance['points'],
                position=position,
    ...
    print('Updating drivers table for the ' + circuits.objects.get(id=seasonCalendar.objects.filter(year=now.year).filter(raceRound=race_round)[0].circuit_id).circuitRef + " grand prix")
    updatePointsTable(race_round)   

def getConstructorStandings(race_round):
    now = datetime.now()
    ...
    def updatePointsTable(race_round):
        ...
        for p in range(0,len(sorted_constructors_by_points_list),1):
            ...
            if constructors.objects.get(id=constructorId).isIncludedInPredictions == 0:
                continue
            else:
                position = position + 1
            ...
            db_entry = constructorStandings(id=id_type, points=instance['points'],
                position=position,
                positionText=instance['positionText'],
                wins=instance['wins'],
                constructor_id=constructorId,
    ...
    try:
        ...
        print("Error capturing constructor standings for round: " + str(race_round))
...
def updateMidfieldPredictionPoints(race_round, paddockId):
    ...
    def captureMidfeildPredictionPoints(uid, r, paddockId):
        ...
        try:
            ...
            pred_points_qset = predictionPoints.objects.filter(
                seasonCalendar_id = season_calendar_id,
                user_id = uid,
                paddock_id = paddockId,
                isFeatureRaceMidfieldPrediction = 1,
            ...
            for pred in range(pred_points_qset.count()):
                instance = predictionPoints.objects.get(id=pred_points_qset[pred].id)

                try:
                    try:
                        forward_looking_instance = predictionPoints.objects.get(id=pred_points_qset[pred + 1].id)
                    except:
                        if instance.finishingPosition == num_drivers and instance.predictedPosition == num_drivers:
                            instance.pointsForPrediction = 2
                            instance.save()
                        continue

                    if instance.predictedPosition == forward_looking_instance.finishingPosition:
                        instance.isFinishingSinglePoint = 1
                        instance.pointsForPrediction = 1
                        forward_looking_instance.isPredictedSinglePoint = 1

                    if instance.finishingPosition == forward_looking_instance.predictedPosition:
                        instance.isPredictedSinglePoint = 1
                        forward_looking_instance.pointsForPrediction = 1
                        forward_looking_instance.isFinishingSinglePoint = 1

                    if instance.finishingPosition > num_drivers:
                        instance.pointsForPrediction = 0
                        instance.isPredictedSinglePoint = 0
                    if instance.predictedPosition > num_drivers:
                        instance.isFinishingSinglePoint = 0
                        instance.pointsForPrediction = 0
    ...
    try:
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        print(exc_type, fname, exc_tb.tb_lineno)
        print("PREDICTION POINTS CAPTURE ERROR")
                                
def updateDriverStandingPredictionPoints(race_round, paddockId):

    def getUserDriverStandingPoints(uid, race_round, paddockId):

        now = datetime.now()

        paddock_rulesId = paddocks.objects.get(id=paddockId).paddockRules_id
        num_drivers = paddockRules.objects.get(id=paddock_rulesId).numDriversOnPreSeasonLeaderboard
        season_calendarId = seasonCalendar.objects.filter(year=now.year).filter(raceRound=race_round)[0].id

        try:
            prediction = driverPredictions.objects.filter(
                user_id=uid,
                isMidSeasonPrediction=0,
                year=now.year,
        ...
        predicted_position_dict['positions'].append(prediction.position1_id)
        predicted_position_dict['positions'].append(prediction.position2_id)
        predicted_position_dict['positions'].append(prediction.position3_id)
        predicted_position_dict['positions'].append(prediction.position4_id)
        predicted_position_dict['positions'].append(prediction.position5_id)
        predicted_position_dict['positions'].append(prediction.position6_id)
        predicted_position_dict['positions'].append(prediction.position7_id)
        predicted_position_dict['positions'].append(prediction.position8_id)
        predicted_position_dict['positions'].append(prediction.position9_id)
        predicted_position_dict['positions'].append(prediction.position10_id)
        predicted_position_dict['positions'].append(prediction.position11_id)
        predicted_position_dict['positions'].append(prediction.position12_id)
        predicted_position_dict['positions'].append(prediction.position13_id)
        predicted_position_dict['positions'].append(prediction.position14_id)
        predicted_position_dict['positions'].append(prediction.position15_id)
        predicted_position_dict['positions'].append(prediction.position16_id)
        predicted_position_dict['positions'].append(prediction.position17_id)
        predicted_position_dict['positions'].append(prediction.position18_id)
        predicted_position_dict['positions'].append(prediction.position19_id)
        predicted_position_dict['positions'].append(prediction.position20_id)
        ...
        for i in range(driver_position_qset.count()):
            
            try:
                driverId = predicted_position_dict['positions'][i]

                try:
                    finishing_position = driver_position_qset.get(
                        seasonCalendar_id = season_calendarId,
                        driver_id = driverId,
                    ).position
                except Exception as e:
        ...
        for pred in range(pred_points_qset.count()):
            ...
            try:
                ...
                if pred+1 <= num_drivers:
                    ...
                    if instance.predictedPosition == instance.finishingPosition:
                        instance.pointsForPrediction = 2
        ...
        print("updated driver standing for user: " + str(uid) + " for round " + str(race_round))
    ...
    for u in range(0, user_qset.count(), 1):
        print("Generating driver standing points for user " + str(user_qset[u].user.username))
        getUserDriverStandingPoints(user_qset[u].user_id, race_round, paddockId)
    ...
    print("Driver Standing records are up to date!")
    print("")
...
def updateConstructorStandingPredictionPoints(race_round, paddockId):
    
    def getUserConstrcutorStandingPoints(uid, race_round, paddockId):

        now = datetime.now()

        paddock_rulesId = paddocks.objects.get(id=paddockId).paddockRules_id
        num_constructors = paddockRules.objects.get(id=paddock_rulesId).numConstructorsOnPreSeasonLeaderboard
        season_calendarId = seasonCalendar.objects.filter(year=now.year).filter(raceRound=race_round)[0].id
        ...
        for i in range(constructor_position_qset.count()):

            constructorId = predicted_position_dict['positions'][i]

            try:
                finishing_position = constructor_position_qset.get(
                    seasonCalendar_id = season_calendarId,
                    constructor_id = constructorId,
                ).position
            except Exception as e:
            ...
            predictionPoint = predictionPoints(
                ...
                user_id=uid,
                isConstructorStandingPrediction=1,
                constructorSeasonPrediction_id=prediction.id,
                paddock_id=paddockId,
            )
            predictionPoint.save()
        ...
        for pred in range(pred_points_qset.count()):
            ...
            try:
                try:
                    ...
                    if instance.finishingPosition == num_constructors and instance.predictedPosition == num_constructors:
                        instance.pointsForPrediction = 2
                        instance.save()
                    continue
                
                if pred+1 <= num_constructors:
                    if instance.predictedPosition == instance.finishingPosition:
                        instance.pointsForPrediction = 2
                    ...
                    if instance.finishingPosition > num_constructors:
                        instance.pointsForPrediction = 0
                        instance.isPredictedSinglePoint = 0
                    if instance.predictedPosition > num_constructors:
                        instance.isFinishingSinglePoint = 0
                        instance.pointsForPrediction = 0
                ...
                elif pred+1 == num_constructors + 1:
                    instance.isFinishingSinglePoint = 0
                    instance.pointsForPrediction = 0
                    instance.isPredictedSinglePoint = 0
                    instance.save()
            ...
            print("User " + str(uid) + " constructor standing record for round " + str(race_round) + " " + save_type + " on the database")
        
        print("updated constructor standing for user: " + str(uid) + " for round " + str(race_round))
    ...
    for u in range(0, user_qset.count(), 1):
        print("Generating constructor standing points for user " + str(user_qset[u].user.username))
        getUserConstrcutorStandingPoints(user_qset[u].user_id, race_round, paddockId)
    ...
    print("Constructor Standing records are up to date!")
    print("")
...
def updateDriverLeaderboards(r, paddockId):
...
def updateConstructorLeaderboards(r, paddockId):
    ...
    for pos in range(paddock_postion_qset.count()):
        ...
        print("Constructor standing Leaderboard records for paddock: " + paddocks.objects.get(id=paddockId).paddockName + " for " + seasonCalendar.objects.get(year=now.year, raceRound=r).circuit.circuitRef)

    seasonCalendarInstance = seasonCalendar.objects.get(id=season_calendarId)
    seasonCalendarInstance.constructorStandingsLeaderboardUpdated = 1
    seasonCalendarInstance.save()

def updateCombinedLeaderBoard(r, paddockId):
    ...
    for u in range(0, paddock_user_qset.count(), 1):
        ...
        previous_position = 1
        round_points = 0
        previous_combined_standing_points
        if r > 1:
        ...
        print("User: " + str(paddock_user_qset[u].user.username) + " combined standing points record for paddock: " + paddocks.objects.get(id=paddockId).paddockName + " Round: " + str(r) + " " + save_type + " the database")
...
def updateAllPredictionPoints():
    ...
    try:
        last_completed_race_round = seasonCalendar.objects.filter(
            isComplete = 1,
            year=now.year,
        ).order_by("-raceRound")[0].raceRound
    except:
    ...
    for race_round in range(next_round_to_capture, next_race_round + 1, 1):
        ...
        getDriverStandings(race_round)
        getConstructorStandings(race_round)
        ...
        for p in range(paddock_qset.count()):
            ...
            if next_round_to_capture >= paddock_driver_start_round and next_round_to_capture <= next_race_round :
                updateDriverStandingPredictionPoints(race_round, paddockId)
                updateDriverLeaderboards(race_round, paddockId)

            if next_round_to_capture >= paddock_constructor_start_round and next_round_to_capture <= next_race_round :
                updateConstructorStandingPredictionPoints(race_round, paddockId)
                updateConstructorLeaderboards(race_round, paddockId)

            updateCombinedLeaderBoard(race_round, paddockId)

        print("SETTING RACE ROUND " + str(race_round) + " TO COMPLETE")
        setRaceRoundToComplete(race_round)
...
def updateRacelyManualPredictionPoints(race_round, paddockId):
    ...
    def captureMidfeildPredictionPoints(uid, r, paddockId):
        ...
        for pred in range(prediction_points_qset.count()):
            instance = manualPredictionPoints.objects.get(id=prediction_points_qset[pred].id)

            try:
                try:
                    forward_looking_instance = manualPredictionPoints.objects.get(id=prediction_points_qset[pred + 1].id)
                except:
                    continue
